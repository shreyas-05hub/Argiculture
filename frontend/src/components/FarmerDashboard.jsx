...
const FarmerDashboard = () => {
  ...
  const callMlModel = async (cropData) => {
    ...
    return {
      ...
      qualityFactors: ["Good Color", "Proper Size", "Fresh Produce"].slice(0, 2 + Math.floor(Math.random() * 2)),
      improvements: Math.random() > 0.7 ? ["Better Packaging", "Harvest Timing"] : []
    };
  };
  ...
  const handleAddCropFromAnimation = async (cropData) => {
    ...
    try {
      ...
      alert(`Crop "${cropData.cropName}" added successfully! AI has graded it as ${mlResult.grade} with suggested price ₹${mlResult.predictedPrice}/kg`);
  ...
  const farmerDeclines = (cropId) => {
    ...
    saveCropsToStorage(updatedLocal);
    alert("You've declined the AI suggestions. You can edit and resubmit the crop later.");
  };
    ...
    <div className="container-fluid mt-4 mb-4">
      ...
      <div className="mb-4">
        <div className="d-flex justify-content-between align-items-center mb-3">
          <h4 className="fw-bold text-success">New Crop Suggestions</h4>
      ...
      <div className="mb-4">
        <div className="d-flex justify-content-between align-items-center mb-3">
          ...
          <span className="badge bg-warning">
            {crops.filter(c => c.status === "Pending" || c.status === "Rejected" || c.status === "Declined").length}
          </span>
        </div>
        <p className="text-muted mb-3">
              ...
              .map((crop) => (
                <div className="col-xl-3 col-lg-4 col-md-6" key={crop.id}>
                  <CropCard
                    ...
                    onAdminReject={() => adminRejects(crop.id, "Quality standards not met")}
                    reason={crop.reason}
                  />
                </div>
              ))}
      ...
      <div className="mb-4">
        <div className="d-flex justify-content-between align-items-center mb-3">
          <h4 className="fw-bold text-success">Sold Crops History</h4>
          <span className="badge bg-success">{crops.filter(c => c.status === "Accepted").length}</span>
        </div>
        <p className="text-muted mb-3">
